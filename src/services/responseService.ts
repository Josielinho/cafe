...
export async function saveAnswer(respuestaEncuestaId: string, preguntaId: string, value: AnswerValue) {
  const basePayload: {
  ...
  if (typeof value === 'object' && value !== null && 'type' in value) {
    ...
    if (value.type === 'multiple_choice') {
      basePayload.respuesta_texto = value.selections.map((selection) => selection.value).join(', ');
    }
  }
...
export async function submitSurveyResponse(
  ...
  const response = await createResponse(encuestaId);

  await Promise.all(answers.map((answer) => saveAnswer(response.id, answer.preguntaId, answer.value)));
